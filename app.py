...
# ─────────────────────────────────────────────────────────────────────
def cargar_logo(ruta) -> str:
    """Devuelve el logo como data URI base64 — sin ninguna request externa."""
    logo_path = Path(ruta) if not isinstance(ruta, Path) else ruta
...
# ─────────────────────────────────────────────────────────────────────
RUTA_DATOS = Path(__file__).parent / "datos" / "datos_demo_template.xlsx"
...
def main():
    ...
    logo_src = cargar_logo(Path(__file__).parent / "assets" / "logo.png")  # ← ruta relativa al proyecto
